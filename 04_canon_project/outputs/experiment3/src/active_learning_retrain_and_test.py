...
def evaluate_folder(gate_ckpt: Path, cls_ckpt: Path, eval_root: Path, image_size: int, gate_threshold: float) -> pd.DataFrame:
    from run_experiment3 import load_model, preprocess_eval, read_image, list_images

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    gate_model = load_model(gate_ckpt, num_classes=2, device=device, task_name="gate", gate_backbone="efficientnet_b0")
    cls_model = load_model(cls_ckpt, num_classes=4, device=device, task_name="classify", gate_backbone="efficientnet_b0")
...
def main() -> None:
    ...
    gate_test = pd.read_csv(paths.artifacts_root / "gate_manifest_test.csv")
    ...
    summary = {
        ...
        "gate_metrics": {
            "best_test_f1_macro": gate_metrics["best_test_f1_macro"],
            "final_test_acc": gate_metrics["final_test_acc"],
            "final_test_f1_macro": gate_metrics["final_test_f1_macro"],
        },
    }
    ...
    md_lines = [
        ...
        f"- best_test_f1_macro: {gate_metrics['best_test_f1_macro']:.4f}",
        f"- final_test_acc: {gate_metrics['final_test_acc']:.4f}",
        f"- final_test_f1_macro: {gate_metrics['final_test_f1_macro']:.4f}",
        "",
