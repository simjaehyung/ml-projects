...
class ProgramConfig:
    ...
    gate_backbone: str = "efficientnet_b0"
    image_size: int = 224
    batch_size: int = 32
    epochs: int = 3
    lr: float = 2e-4
...
class BaselineWorkflowProgram:
    ...
    def _handle_state(self, state: WorkflowState, context: Dict[str, Any]) -> None:
        ...
        if state == WorkflowState.AUG_DONE:
            ...
            gate_train, gate_test = split_manifest_train_test(gate_df, train_ratio=cfg.train_ratio, seed=cfg.seed)
            cls_train, cls_test = split_manifest_train_test(cls_df, train_ratio=cfg.train_ratio, seed=cfg.seed)
        ...
        if state == WorkflowState.BASE_TRAIN_DONE:
            ...
            gate_test = paths.artifacts_root / "gate_manifest_test.csv"
            cls_train = paths.artifacts_root / "classify_manifest_train.csv"
            cls_test = paths.artifacts_root / "classify_manifest_test.csv"
            ...
            gate_metrics = train_one_task(
                ...
                lr=cfg.lr,
                seed=cfg.seed,
                gate_backbone=cfg.gate_backbone,
            )
            cls_metrics = train_one_task(
                task_name="classify",
                train_df=pd.read_csv(cls_train),
                test_df=pd.read_csv(cls_test),
                num_classes=4,
                paths=paths,
                image_size=cfg.image_size,
                batch_size=cfg.batch_size,
