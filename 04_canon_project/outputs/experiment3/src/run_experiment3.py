...
from torchvision.models import EfficientNet_B0_Weights, EfficientNet_B1_Weights, EfficientNet_B2_Weights, EfficientNet_B3_Weights, ResNet18_Weights

IMAGE_EXTS = {".jpg", ".jpeg", ".png", ".bmp", ".webp"}
CLASS_NAMES = ["t1", "t2", "t3", "t4"]
SUPPORTED_EFFICIENTNET_B = ("efficientnet_b0", "efficientnet_b1", "efficientnet_b2", "efficientnet_b3")
...
def split_manifest_train_test(df: pd.DataFrame, train_ratio: float, seed: int) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for label_id, group in df.groupby("label_id"):
        if len(group) < 2:
            raise RuntimeError(f"Need at least 2 samples per class for split, label_id={label_id} has {len(group)}")

        shuffled = group.sample(frac=1.0, random_state=seed + int(label_id)).reset_index(drop=True)
        ...
        test_parts.append(shuffled.iloc[train_count:])
...
def create_gate_model(num_classes: int, device: torch.device, gate_backbone: str) -> nn.Module:
    gate_backbone = gate_backbone.lower()
    if gate_backbone == "efficientnet_b0":
        model = models.efficientnet_b0(weights=EfficientNet_B0_Weights.DEFAULT)
    elif gate_backbone == "efficientnet_b1":
        model = models.efficientnet_b1(weights=EfficientNet_B1_Weights.DEFAULT)
    elif gate_backbone == "efficientnet_b2":
        model = models.efficientnet_b2(weights=EfficientNet_B2_Weights.DEFAULT)
    elif gate_backbone == "efficientnet_b3":
        model = models.efficientnet_b3(weights=EfficientNet_B3_Weights.DEFAULT)
    else:
        raise ValueError(f"Unsupported gate_backbone={gate_backbone}. Choose one of {SUPPORTED_EFFICIENTNET_B}")
...
def create_classifier_model(num_classes: int, device: torch.device) -> nn.Module:
...
def train_one_task(
    ...
    if task_name.startswith("gate"):
        model = create_gate_model(num_classes, device, gate_backbone=gate_backbone)
    else:
        model = create_classifier_model(num_classes, device)

    train_ds = ImagePathDataset(train_df, image_size=image_size)
    test_ds = ImagePathDataset(test_df, image_size=image_size)
    ...
    for epoch in range(1, epochs + 1):
        ...
        history.append({"epoch": epoch, "train_loss": train_loss, "test_acc": acc, "test_f1_macro": f1})
    ...
    result = {
        "task": task_name,
        "device": str(device),
        "best_test_f1_macro": best_test_f1,
        "final_test_acc": float(history[-1]["test_acc"]),
        "final_test_f1_macro": float(history[-1]["test_f1_macro"]),
        "history": history,
...
def load_model(ckpt_path: Path, num_classes: int, device: torch.device, task_name: str, gate_backbone: str) -> nn.Module:
    if task_name.startswith("gate"):
        model = create_gate_model(num_classes, device, gate_backbone=gate_backbone)
    else:
        model = create_classifier_model(num_classes, device)
...
def evaluate_3fps(
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    gate_model = load_model(gate_ckpt, num_classes=2, device=device, task_name="gate", gate_backbone=gate_backbone)
    cls_model = load_model(cls_ckpt, num_classes=4, device=device, task_name="classify", gate_backbone=gate_backbone)
...
def write_report(
    ...
    report_lines = [
        ...
        f"- Train/Test split ratio on augmented set: {train_ratio:.2f}/{1.0 - train_ratio:.2f}",
        "- Gate uses wider real-world perturbations; Classification uses conservative perturbations.",
        "",
        "## 3. Augmented Image Holdout Test Results",
        f"- Gate train/test count: {gate_metrics['train_count']} / {gate_metrics['test_count']}",
        f"- Gate best test macro-F1: {gate_metrics['best_test_f1_macro']:.4f}",
        f"- Gate final test acc: {gate_metrics['final_test_acc']:.4f}",
        f"- Classification train/test count: {cls_metrics['train_count']} / {cls_metrics['test_count']}",
        f"- Classification best test macro-F1: {cls_metrics['best_test_f1_macro']:.4f}",
        f"- Classification final test acc: {cls_metrics['final_test_acc']:.4f}",
        "",
        "## 4. Video (3fps) Evaluation",
    ...
    report_lines.extend(
        [
            ...
            "",
            "## 6. Notes",
            "- This pipeline was built from scratch in experiment3 and does not import any legacy training code.",
            "- If needed, you can increase epochs for stronger convergence once the strategy is validated.",
        ]
    )

    out_path = paths.reports_root / "EXPERIMENT3_REPORT.md"
    out_path.write_text("\n".join(report_lines), encoding="utf-8")


def parse_args() -> argparse.Namespace:
    default_experiment_root = Path(__file__).resolve().parents[1]
    parser = argparse.ArgumentParser(description="Experiment3: offline augmentation + EfficientNet-B gate + classify training + 3fps eval")
    ...
    parser.add_argument("--gate-backbone", type=str, default="efficientnet_b0", choices=list(SUPPORTED_EFFICIENTNET_B))
...
def main() -> None:
    ...
    gate_train, gate_test = split_manifest_train_test(gate_df, train_ratio=args.train_ratio, seed=args.seed)
    cls_train, cls_test = split_manifest_train_test(cls_df, train_ratio=args.train_ratio, seed=args.seed)

    gate_train.to_csv(paths.artifacts_root / "gate_manifest_train.csv", index=False)
    gate_test.to_csv(paths.artifacts_root / "gate_manifest_test.csv", index=False)
    cls_train.to_csv(paths.artifacts_root / "classify_manifest_train.csv", index=False)
    cls_test.to_csv(paths.artifacts_root / "classify_manifest_test.csv", index=False)
    ...
    gate_metrics = train_one_task(
        ...
        lr=args.lr,
        seed=args.seed,
        gate_backbone=args.gate_backbone,
    )
    ...
    print("Experiment3 pipeline completed.")
    print(f"Report: {paths.reports_root / 'EXPERIMENT3_REPORT.md'}")
